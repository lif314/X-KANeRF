...
| √ | [Gegenbauer polynomials](https://en.wikipedia.org/wiki/Gegenbauer_polynomials) | $$C_n^{(\lambda)}(x) = \frac{(2\lambda)_n}{n!} \sum_{k=0}^{\lfloor \frac{n}{2} \rfloor} (-1)^k \frac{\Gamma(n-k+2\lambda)}{k! (n-2k)!} \left(2x\right)^{n-2k}$$  | [OrthogPolyKANs](https://github.com/Boris-73-TA/OrthogPolyKANs) |
